---
title: "Guition ESP32 2.8\" LCD Display JC2432W328C"
description: "ESP32 development board with 2.8-inch 240x320 LCD, capacitive touchscreen, WiFi and Bluetooth"
category: "board"
manufacturer: "Guition"
model: "JC2432W328C"
connectionTypes: ["gpio", "i2c", "spi"]
tags: ["wifi", "bluetooth", "esp32", "lcd", "touchscreen", "display"]
productionStatus: "active"
status: "pending"
references:
  - title: Download Software & Examples
    url: http://pan.jczn1688.com/s/6brhsi
  - title: ESP32 Cheap Yellow Display Project
    url: https://github.com/witnessmenow/ESP32-Cheap-Yellow-Display
  - title: Module (Sparkle IOT XH-32S)
    url: https://fccid.io/2BCLR-XH32S/
  - title: Similar ESP32 Board Reference
    url: https://www.espboards.dev/esp32/esp32doit-devkit-v1/
purchaseLinks:
  - vendor: AliExpress
    url: https://www.aliexpress.com/item/1005006732002132.html
  - vendor: AliExpress (Alternative)
    url: https://www.aliexpress.com/item/1005007865384573.html
image: "/images/devices/jc2432w328c.png"
count: 2
usedCount: 1
---

## Overview

The Guition ESP32 JC2432W328C is a feature-rich development board combining an ESP32 module with a 2.8-inch 240x320 color LCD display and capacitive touchscreen. It includes WiFi, Bluetooth, RGB LED, speaker connector, SD card slot, photoresistor, and battery support, making it ideal for IoT projects requiring a user interface.

The board uses the **Sparkle IOT XH-32S** module, which is based on the ESP32-WROOM-32 with a dual-core Tensilica LX6 MCU running at 240MHz with 4MB Flash memory.

## Hardware Features

- **Display:** 2.8" TFT LCD 240x320 (ST7789V controller)
- **Touchscreen:** Capacitive touch (CST816 controller)
- **Connectivity:** WiFi 802.11 b/g/n HT40, Bluetooth/BLE 4.2
- **Storage:** TF/Micro SD card slot
- **Audio:** Speaker connector (GPIO26)
- **Sensors:** Photoresistor (GPIO34)
- **LED:** RGB LED (GPIO4=Red, GPIO16=Green, GPIO17=Blue)
- **Power:** USB-C for programming and power, battery interface with switch, 4P 1.25 power supply base
- **Buttons:** Boot, Reset, Battery/External power switch
- **Backlight Control:** GPIO27 (PWM controllable)

### Module Specifications

- **Module:** Sparkle IOT XH-32S (similar to ESP32-WROOM-32)
- **CPU:** Dual-core Xtensa 32-bit LX6, up to 240MHz
- **RAM:** 520KB SRAM
- **Flash:** 4MB
- **WiFi:** 802.11 b/g/n HT40
- **Bluetooth:** v4.2 BR/EDR and BLE
- **Operating Voltage:** 3.3V
- **Input Voltage:** 5V (USB-C or external)

## Pinout and I/O Connectors

![Back of the board](/images/devices/jc2432w328c.png)

### Extended I/O Connectors

Three 4-pin connectors provide access to additional GPIOs:

1. **Connector 1:** GND, IO16, IO4, IO17 (RGB LED connections)
2. **Connector 2:** GND, IO22, IO21, VCC (I2C interface)
3. **Connector 3:** GND, IO32, IO22, IO21 (I2C + Touch I2C)

### Pin Distribution

| Pin | ESP32 Pin | Function | Connected To | Notes |
|-----|-----------|----------|--------------|-------|
| 1 | IO0 | BOOT | Boot button | |
| 2 | IO1 | U0TXD | Serial TX | |
| 3 | IO2 | GPIO | TFT_RS | Display DC pin |
| 4 | IO3 | U0RXD | Serial RX | |
| 5 | IO4 | GPIO | LED1_2 | RGB LED - Red |
| 6 | IO5 | VSPI CS0 | TF_CS | SD Card CS |
| 7 | IO6 | SCK | Flash | Don't use |
| 8 | IO7 | SDO | Flash | Don't use |
| 9 | IO8 | SDI | Flash | Don't use |
| 10 | IO9 | SHD/SD2 | Flash | Don't use |
| 11 | IO10 | SWP/SD3 | Flash | Don't use |
| 12 | IO11 | CSC/CMD | Flash | Don't use |
| 13 | IO12 | HSPI MISO | TFT_SDO | Display SPI MISO |
| 14 | IO13 | HSPI MOSI | TFT_SDI | Display SPI MOSI |
| 15 | IO14 | HSPI CLK | TFT_SCK | Display SPI CLK |
| 16 | IO15 | HSPI CS0 | TFT_CS | Display SPI CS |
| 17 | IO16 | U2TXD | LED1_3 | RGB LED - Green |
| 18 | IO17 | U2RXD | LED1_1 | RGB LED - Blue |
| 19 | IO18 | VSPI CLK | TF_CLK | SD Card CLK |
| 20 | IO19 | VSPI MISO | TF_SDO | SD Card MISO |
| 21 | IO21 | I2C SDA | I2C Bus | |
| 22 | IO22 | I2C SCL | I2C Bus | |
| 23 | IO23 | VSPI MOSI | TF_SDI | SD Card MOSI |
| 24 | IO25 | GPIO | CTP_RST | Touch Reset |
| 25 | IO26 | GPIO | SPK | Speaker |
| 26 | IO27 | GPIO | BL_CTRL | Display Backlight PWM |
| 27 | IO32 | GPIO | CTP_SCL | Touch I2C SCL |
| 28 | IO33 | GPIO | CTP_SDA | Touch I2C SDA |
| 29 | IO34 | INPUT ONLY | Photoresistor | Analog input |
| 30 | IO35 | INPUT ONLY | - | Available |
| 31 | IO36 | INPUT ONLY | - | Available |
| 32 | IO39 | INPUT ONLY | - | Available |

## ESPHome Configuration

### Complete Example Configuration

```yaml
esphome:
  name: guition-display

esp32:
  board: esp32dev
  framework:
    type: esp-idf

# Sensor for onboard photoresistor
sensor:
  - platform: adc
    pin: GPIO34
    name: "Ambient Light"
    id: board_ldr
    update_interval: 60s
    attenuation: auto

# PWM outputs for RGB LED and backlight
output:
  - platform: ledc
    pin: GPIO04
    id: rgb_led_red
    inverted: true
  - platform: ledc
    pin: GPIO16
    id: rgb_led_green
    inverted: true
  - platform: ledc
    pin: GPIO17
    id: rgb_led_blue
    inverted: true
  - platform: ledc
    pin: GPIO27
    id: backlight_pwm

# RGB LED and backlight control
light:
  - platform: monochromatic
    output: backlight_pwm
    name: "Display Backlight"
    id: backlight
    restore_mode: ALWAYS_ON
  - platform: rgb
    id: internal_rgb
    name: "RGB LED"
    red: rgb_led_red
    green: rgb_led_green
    blue: rgb_led_blue
    restore_mode: ALWAYS_OFF

# Boot button
binary_sensor:
  - platform: gpio
    pin:
      number: GPIO00
      inverted: true
    name: "Boot Button"
    id: boot_btn

# SPI buses - separate for display and SD card
spi:
  - id: spi_card
    clk_pin: GPIO18
    mosi_pin: GPIO23
    miso_pin: GPIO19
  - id: spi_display
    clk_pin: GPIO14
    mosi_pin: GPIO13
    miso_pin: GPIO12

# Display configuration
display:
  - platform: mipi_spi
    model: ST7789V
    id: my_display
    spi_id: spi_display
    cs_pin: GPIO15
    dc_pin: GPIO02
    data_rate: 40MHz
    rotation: 90
    auto_clear_enabled: false
    update_interval: never
    lambda: |-
      // Your drawing code here
      it.print(0, 0, id(font), "Hello World!");

# I2C bus for touchscreen
i2c:
  id: i2c_bus
  sda: GPIO33
  scl: GPIO32

# Capacitive touchscreen
touchscreen:
  - platform: cst816
    id: my_touchscreen
    #NOTE: do not setup interrupt pin otherwise weird behavior
    reset_pin: GPIO25
    transform:
      swap_xy: true
      mirror_y: true
      mirror_x: false
    on_touch:
      - lambda: |-
          ESP_LOGI("touch", "x=%d, y=%d", touch.x, touch.y);

# Optional: Speaker output
#output:
#  - platform: ledc
#    pin: GPIO26
#    id: speaker_out
#
#rtttl:
#  output: speaker_out
```

### SD Card Configuration

To use the SD card slot:

```yaml
spi:
  - id: spi_card
    clk_pin: GPIO18
    mosi_pin: GPIO23
    miso_pin: GPIO19

# Note: SD card support in ESPHome may require additional configuration
# Refer to ESPHome documentation for SD card component usage
```
**NOTE**: I haven't tested SD Card yet.

## Touchscreen Calibration

The touchscreen may require calibration. Use the following code to display raw touch coordinates:

```yaml
touchscreen:
  - platform: cst816
    id: my_touchscreen
    reset_pin: GPIO25
    on_touch:
      - lambda: |-
          ESP_LOGI("calibration", "x=%d, y=%d, x_raw=%d, y_raw=%d",
              touch.x, touch.y, touch.x_raw, touch.y_raw);
```

Adjust the `transform` parameters based on the output:
- `swap_xy`: Swap X and Y coordinates
- `mirror_x`: Mirror horizontally
- `mirror_y`: Mirror vertically

## Important Notes

⚠️ **SPI Buses:** The board uses two separate SPI buses - one for the display (HSPI) and one for the SD card (VSPI). Don't mix them up.

⚠️ **Flash Pins:** GPIO6-11 are connected to the flash memory. Do not use these pins.

⚠️ **Input-Only Pins:** GPIO34-39 can only be used as inputs and don't have internal pull-up/pull-down resistors.

⚠️ **RGB LED:** The RGB LED pins are inverted (active low).

⚠️ **Touch Reset:** GPIO25 is used for touchscreen reset. Must be configured in the touchscreen component.

⚠️ **Display Updates:** For smooth operation with LVGL or similar graphics libraries, set `update_interval: never` and `auto_clear_enabled: false` in the display config.

## Using with LVGL

This board works great with LVGL (Light and Versatile Graphics Library). To wake the display on touch:

```yaml
touchscreen:
  - platform: cst816
    id: my_touchscreen
    reset_pin: GPIO25
    transform:
      swap_xy: true
      mirror_y: true
    on_release:
      - if:
          condition: lvgl.is_paused
          then:
            - logger.log: "LVGL resuming"
            - lvgl.resume:
            - lvgl.widget.redraw:
            - light.turn_on: backlight
```

## Troubleshooting

**Display not working:**
- Check backlight is enabled (GPIO27)
- Verify SPI pins are correct (HSPI bus)
- Try lower data_rate (start with 10MHz, then increase)

**Touch not responding:**
- Verify I2C pins (GPIO32=SCL, GPIO33=SDA)
- Check touchscreen reset pin (GPIO25) is configured
- Do not configure interrupt pin
- Run I2C scan to confirm CST816 is detected

**Board won't flash:**
- Hold the BOOT button while connecting USB
- Try a different USB cable (must support data)
- Ensure USB drivers are installed
- It uses CH340C - I noticed only one serial port showing up even when multiple boards connected, disconnect other boards

**Random resets:**
- Check power supply (needs stable 5V, 500mA+)
- If using battery, ensure it's charged
- Disable WiFi temporarily to test if power-related

